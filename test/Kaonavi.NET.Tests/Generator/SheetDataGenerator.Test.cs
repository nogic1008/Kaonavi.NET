...
public sealed class SheetDataGeneratorTest
{
    ...
    """, "KAONAVI002", DisplayName = $"Generator > 入れ子にされたクラスの場合、KAONAVI002のコンパイル警告が発生する。")]
    [DataRow("""
    using Kaonavi.Net;
    
    [SheetSerializable]
    public partial record Foo(string Code, [property: CustomField(101)] string Name);
    """, "KAONAVI003", DisplayName = $"Generator > {nameof(ISheetData)}を継承していない場合、KAONAVI003のコンパイル警告が発生する。")]
    [DataRow("""
    using Kaonavi.Net;
    ...
    """, "KAONAVI004", DisplayName = $"Generator > {nameof(ISheetData.ToCustomFields)}メソッドを手動で実装している場合、KAONAVI004のコンパイル警告が発生する。")]
    [DataRow("""
    using Kaonavi.Net;
    
    [SheetSerializable]
    public partial record Foo(string Code, string Name) : ISheetData;
    """, "KAONAVI005", DisplayName = $"Generator > {nameof(CustomFieldAttribute)}を設定したプロパティがない場合、KAONAVI005のコンパイル警告が発生する。")]
    [DataRow("""
    using Kaonavi.Net;
    ...
    """, "KAONAVI006", DisplayName = $"Generator > {nameof(CustomFieldAttribute)}のidが重複したプロパティがある場合、KAONAVI006のコンパイル警告が発生する。")]
    [DataRow("""
    using Kaonavi.Net;
    ...
    """, "KAONAVI007", DisplayName = $"Generator > {nameof(CustomFieldAttribute)}属性を持つプロパティがgetアクセサーを持っていない場合、KAONAVI007のコンパイル警告が発生する。")]
