...
public sealed class SheetDataGeneratorTest
{
    ...
    public static IEnumerable<TestDataRow<(string code, string id)>> InvalidCodeTestData
    {
        get
        {
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > partial クラスでない場合、KAONAVI001 のコンパイル警告が発生する。",
            };
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > 入れ子にされたクラスの場合、KAONAVI002 のコンパイル警告が発生する。",
            };
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > {nameof(ISheetData)} を継承していない場合、KAONAVI003 のコンパイル警告が発生する。",
            };
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > {nameof(ISheetData.ToCustomFields)} メソッドを手動で実装している場合、KAONAVI004 のコンパイル警告が発生する。",
            };
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > {nameof(CustomFieldAttribute)} を設定したプロパティがない場合、KAONAVI005 のコンパイル警告が発生する。",
            };
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > {nameof(CustomFieldAttribute)} の id が重複したプロパティがある場合、KAONAVI006 のコンパイル警告が発生する。",
            };
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > {nameof(CustomFieldAttribute)} 属性を持つプロパティが get アクセサーを持っていない場合、KAONAVI007 のコンパイル警告が発生する。",
            };
        }
    }
    ...
    public async Task When_Invalid_Code_Compiler_Warns_With_Diagnostic(string code, string id)
    {
        ...
        // Assert
        await Assert.That(warnings).Contains(d => d.Id == id);
    }
    ...
    public static IEnumerable<
        TestDataRow<(LanguageVersion version, string code, string expected)>
    > ValidCodeTestData
    {
        get
        {
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > C# 9.0 の場合、ネストされた名前空間 および 配列の初期化子を使用してソース生成する。",
            };
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > C# 11.0 の場合、ファイルスコープ名前空間 および 配列の初期化子を使用してソース生成する。",
            };
            ...
            {
                DisplayName =
                    $"{nameof(SheetDataGenerator)} > C# 12.0 の場合、ファイルスコープ名前空間 および コレクション式を使用してソース生成する。",
            };
        }
    }
    ...
    [Test]
    [MethodDataSource(nameof(ValidCodeTestData))]
    public async Task Generates_ToCustomFields_Method(
    ...
    )
    {
        ...
        await Assert
            .That(sb.ToString().ReplaceLineEndings("\n"))
