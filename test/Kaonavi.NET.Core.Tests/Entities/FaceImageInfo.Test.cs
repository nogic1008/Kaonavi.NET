...
public sealed class FaceImageInfoTest
{
    ...
    [DisplayName($"{nameof(FaceImageInfo)} > JSONからデシリアライズできる。")]
    public async Task CanDeserializeJSON()
    {
        ...
        await Assert
            ...
            .And.Member(
                static o => o.DownloadUrl,
                static o =>
                    o.IsEqualTo(
                        new(
                            "https://example.kaonavi.jp/image/xxxx.jpg?Expires=1755255000&Signature=xxxx&Key-Pair-Id=EXAMPLEKEYPAIRID"
                        )
                    )
            )
            .And.Member(
                static o => o.UpdatedAt,
                static o => o.IsEqualTo(new(2020, 10, 1, 1, 23, 45))
            );
    }
}
