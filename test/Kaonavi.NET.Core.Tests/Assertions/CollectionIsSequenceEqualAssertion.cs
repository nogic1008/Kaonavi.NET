...
public class CollectionIsSequenceEqualAssertion<TCollection, TItem>(AssertionContext<TCollection> context, IEnumerable<TItem> expected)
    : CollectionAssertionBase<TCollection, TItem>(context)
    where TCollection : IEnumerable<TItem>
{
    private readonly IEnumerable<TItem> _expected = expected ?? throw new ArgumentNullException(nameof(expected));
    ...
    protected override Task<AssertionResult> CheckAsync(EvaluationMetadata<TCollection> metadata)
    {
        ...
        return Task.FromResult(AssertionResult.Failed($"'{string.Join(", ", value)}' does not equal '{string.Join(", ", _expected)}'"));
    }
}
